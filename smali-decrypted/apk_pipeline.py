...
def run_apk_pipeline(sample_apk: str = SAMPLE_APK, nested_apk: str = NESTED_APK, password: str = PASSWORD):
    """
    sample.apk / nested apk를 대상으로 전체 복호화/우회/재암호화/재서명 파이프라인을 실행.
    예외는 그대로 상위로 올리고, 성공 시 keystore 경로를 반환.
    """
